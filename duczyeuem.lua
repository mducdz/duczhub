        ...
        if _G.Level then
            pcall(function()
                ...
                elseif inSub or level < 2600 then
                    ...
                    -- [FIX] Nhan quest neu chua co (KHONG abandon lung tung nua)
                    if not questUI.Visible then
                        ...
                        task.wait(0.6)
                        -- Khong return: tiep tuc bay den mob ngay trong vong nay
                    end

                    -- [FIX] Luon bay den mob de farm
                    local target = nil
                    ...
                    for _, v in pairs(workspace.Enemies:GetChildren()) do
                        if v.Name == enemyName and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                            ...
                            if d < best then best = d target = v end
                        end
                    end
                    ...
                    if target then
                        repeat
                            ...
                            if target:FindFirstChild("HumanoidRootPart") then
                                _tp(GetSpinCFrame(target.HumanoidRootPart, GetFarmOffset()))
                            end
                            Attack.Kill(target, _G.Level)
                        until not _G.Level or not target.Parent or not target:FindFirstChild("Humanoid") or target.Humanoid.Health <= 0
                    else
                        -- Khong thay mob -> bay den diem spawn cua mob
                        local spawnCF = nil
                        pcall(function()
                            for _, sp in pairs(workspace["_WorldOrigin"].EnemySpawns:GetChildren()) do
                                if string.find(sp.Name, enemyName) then
                                    spawnCF = sp.CFrame
                                    break
                                end
                            end
                        ...
                        if not spawnCF then
                            pcall(function()
                                for _, v in pairs(replicated.Enemies:GetChildren()) do
